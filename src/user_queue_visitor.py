...
def visitor_queue(root, button_text, select_student, purpose):
    ...
    user_visitor.iconbitmap("C:\\Users\\QHTF\\OneDrive\\Desktop\\new_queue_system\\QMS-python-gui-main\\old-logo.ico")
    ...
    def create_ticket(button_text, select_student, purpose):
        ...
        try:
            ...
            check_name = "SELECT full_name FROM queue WHERE full_name = %s"
            cursor.execute(check_name, (visitor_name,))
            result_name = cursor.fetchall()
            if result_name:
                messagebox.showwarning("Info", "Already have a ticket")
                return
